        ...
        <div class="container-xxl py-5">
            <div class="container">
                <div class="text-center mx-auto mb-5 wow fadeInUp" data-wow-delay="0.1s" style="max-width: 600px;">
					...
					<h3><p align='center'>Bilgilerinizi Değiştirmek İçin <a href='bilgidegis.php' title='Çıkış'>Tıklayınız.</p></a></h3>
				</div>
				...
				<?php
    				if(@$_SESSION["name"]) {
    					...
    					while ($gudoldur = mysqli_fetch_array($gusorgu)) {
                            ...
                            if ($gudoldur['tur'] == '1') {
                                echo "<a href='admin\index.php?id=".$kodid."'>Admin Panel</a>";
                            }
    					}
    					$siparis = mysqli_query($conn, "select * from uyeli_sepet where uye_id ='".$id."'");
    					echo "<center><h1 class='mb-3'><font color='#0f7791'>Siparişlerim.</font></h1></center>";
    					while ($sdoldur = mysqli_fetch_array($siparis)) {

    						$urunler = $sdoldur['urun_isimler'];
					        ...
					        foreach ($blocks as $block) {
					            ...
					            while ($doldur = mysqli_fetch_array($usorgu)) {
					                $urunfiyatdizi[] = $doldur['urun_fiyat'];
					            }
					        }
					        $length = count($blocks);
					        $text2 = $sdoldur['adet'];
					        ...
					        $urunresim = $sdoldur['urun_resimler'];
					        $urunresimdizi = array();
							$images = explode(';', $urunresim);
							foreach ($images as $image) {
							    $urunresimdizi[] = $image;
							}

					        echo "<div class='table-wrapper' style='margin-left: 250px'>";
					        for ($i=0; $i < $length - 1; $i++) {
					        	...
					        	echo "
    								...
    								<tbody>
    								<tr>
    									<td><img src='".$urunresimdizi[$i]."' style='width:50px; height:50px' ></td>
	    								<td>$urundizi[$i] &nbsp;</td> 
	    								<td>$urunfiyatdizi[$i] &nbsp;</td>"; 
	    								if (empty($adetler[$i])) {
                                            echo "<td>1 &nbsp;</td>";
                                        } else {
                                            echo "<td>$adetler[$i] &nbsp;</td>";
                                        }
                                        echo "<td>".$sdoldur['durum']."</td>";
    								echo "</tr>
    								</tbody>
    								</table>
    							";
